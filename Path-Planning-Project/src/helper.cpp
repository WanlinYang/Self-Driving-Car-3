...
vector<double> map2carCoord(double x, double y, double theta){
  double x_car = x*cos(-theta) - y*sin(-theta);
  double y_car = x*sin(-theta) + y*cos(-theta);
  return {x_car, y_car};
}
vector<double> car2mapCoord(double x, double y, double theta){
  double x_map = x*cos(theta) - y*sin(theta);
  double y_map = x*sin(theta) + y*cos(theta);
  return {x_map, y_map};
}
